...
function initials(name) {
  return String(name || "Paciente").trim().split(/\s+/).slice(0,2).map(p => p[0]?.toUpperCase()).join("") || "P";
}
...
function cloudinaryReady() {
  ...
  return Boolean(c.cloudName && c.uploadPreset && !String(c.cloudName).includes("COLE_") && !String(c.uploadPreset).includes("COLE_"));
}
...
function sanitizePublicId(value) {
  return String(value || "arquivo").replace(/[^a-zA-Z0-9_\-\/]/g, "_").replace(/_+/g, "_");
}
async function uploadPdfCloudinary(fileOrBlob, { receitaId, tipo }) {
  ...
  if (c.folder) form.append("folder", sanitizePublicId(c.folder));
...
async function signIn() {
  ...
  if (!email || !senha) return setStatus("Informe e-mail e senha.", "warn");
  try {
...
async function loadMedicoPerfil() {
  ...
  if (snap.exists()) medicoPerfil = snap.val();
  else {
    medicoPerfil = {
...
async function savePerfil() {
  ...
  await db.ref(`dentistas/${currentUser.uid}/perfil`).set(medicoPerfil);
  fillPerfilForm();
...
async function savePaciente() {
  ...
  if (!nome) return setStatus("Informe o nome do paciente.", "warn");
  ...
  await db.ref(`pacientes/${currentUser.uid}/${id}`).set(paciente);
  clearPacienteForm();
  setStatus("Paciente salvo.", "blue");
}
function clearPacienteForm() {
  ["#pacId","#pacNome","#pacCpf","#pacNascimento","#pacTelefone","#pacEmail","#pacEndereco"].forEach(s => $(s).value = "");
}
function editPaciente(id) {
  const p = pacientesCache[id]; if (!p) return;
  $("#pacId").value = id;
...
function renderPacientes() {
  ...
  list.forEach(p => {
    ...
    tr.innerHTML = `
      <td><strong>${safeText(p.nome)}</strong><br><span class="badge">${safeText(p.telefone || "sem telefone")}</span></td>
      <td>${safeText(p.cpf || "-")}</td>
      <td>${safeText(p.nascimento || "-")}</td>
      <td>${safeText(p.email || "-")}</td>
      <td><button class="btn ghost" onclick="editPaciente('${p.id}')">Editar</button> <button class="btn red" onclick="deletePaciente('${p.id}')">Excluir</button></td>`;
    body.appendChild(tr);
...
function collectReceitaForm(id = null) {
  ...
  if (!paciente) throw new Error("Selecione um paciente cadastrado.");
  const tipo = $("#recTipo").value;
  const medicamento = $("#recMedicamento").value.trim();
  const posologia = $("#recPosologia").value.trim();
  if (!medicamento || !posologia) throw new Error("Informe medicamento e posologia.");
...
function clearReceitaForm() {
  ["#recPaciente","#recMedicamento","#recConcentracao","#recForma","#recQuantidade","#recPosologia","#recDuracao","#recOrientacoes"].forEach(s => $(s).value = "");
...
async function generatePdfBlob(r, signedLabel = false) {
  ...
  doc.setFontSize(9);
  doc.text(doc.splitTextToSize(medicoPerfil.endereco || "", 174), margin, y); y += medicoPerfil.endereco ? 8 : 2;
  doc.text(`Cirurgião-dentista: ${medicoPerfil.nome || ""}   CRO: ${medicoPerfil.cro || ""}/${medicoPerfil.uf || ""}${medicoPerfil.rqe ? `   Especialidade: ${medicoPerfil.rqe}` : ""}`, margin, y); y += 9;
  doc.setDrawColor(180); doc.line(margin, y, 210-margin, y); y += 10;

  doc.setFont("helvetica", "bold"); doc.setFontSize(18);
  const title = r.tipo === "ATESTADO" ? "ATESTADO ODONTOLÓGICO" : r.tipo === "PEDIDO_EXAME" ? "PEDIDO DE EXAME" : "RECEITA ODONTOLÓGICA";
  doc.text(title, 105, y, { align: "center" }); y += 12;

  doc.setFont("helvetica", "normal"); doc.setFontSize(11);
  doc.text(`Receita ID: ${r.id}`, margin, y); y += 7;
  doc.text(`Data de emissão: ${new Date(r.dataEmissao).toLocaleString("pt-BR")}`, margin, y); y += 8;
  doc.setFont("helvetica", "bold"); doc.text("Paciente", margin, y); y += 6;
  doc.setFont("helvetica", "normal");
  doc.text(`Nome: ${r.paciente.nome || ""}`, margin, y); y += 6;
  doc.text(`CPF: ${r.paciente.cpf || "não informado"}   Nascimento: ${r.paciente.nascimento || "não informado"}`, margin, y); y += 8;

  doc.setFont("helvetica", "bold"); doc.text("Prescrição / Documento", margin, y); y += 7;
  doc.setFont("helvetica", "normal");
  const linhas = [
    `Tipo: ${r.tipo}`,
  ...
  y += 8;
  doc.setDrawColor(120); doc.line(55, y, 155, y); y += 6;
  doc.setFont("helvetica", "bold"); doc.text(medicoPerfil.nome || "Cirurgião-dentista", 105, y, { align: "center" }); y += 5;
  doc.setFont("helvetica", "normal"); doc.text(`CRO ${medicoPerfil.cro || ""}/${medicoPerfil.uf || ""}`, 105, y, { align: "center" }); y += 10;

  const valUrl = validationUrl(r.id);
  doc.setFontSize(9);
  doc.setTextColor(70);
  doc.text("Validação:", margin, y); y += 5;
  doc.text(doc.splitTextToSize(valUrl, 174), margin, y); y += 8;
  doc.setTextColor(0);
  doc.setFont("helvetica", "bold");
  if (signedLabel) {
...
async function gerarReceita() {
  try {
    ...
    const upload = await uploadPdfCloudinary(blob, { receitaId: r.id, tipo: "original-nao-assinada" });
    r.pdfOriginalUrl = upload.secureUrl;
    r.cloudinaryOriginal = upload;
    await db.ref(`receitasPrivadas/${currentUser.uid}/${r.id}`).set(r);
...
function clearReceitaInputsAfterSave(){
  ["#recMedicamento","#recConcentracao","#recForma","#recQuantidade","#recPosologia","#recDuracao","#recOrientacoes"].forEach(s => $(s).value = "");
}
...
async function uploadPdfAssinado(receitaId, inputId) {
  ...
  if (!file) return setStatus("Selecione o PDF assinado ICP-Brasil.", "warn");
  if (file.type !== "application/pdf" && !file.name.toLowerCase().endsWith(".pdf")) return setStatus("Envie apenas PDF.", "warn");
  const receitaSnap = await db.ref(`receitasPrivadas/${currentUser.uid}/${receitaId}`).get();
  if (!receitaSnap.exists()) return setStatus("Receita não encontrada.", "danger");
  const hash = await sha256File(file);
  const upload = await uploadPdfCloudinary(file, { receitaId, tipo: "assinada-icp" });
  const updates = {
  ...
  const merged = { ...receitaSnap.val(), ...updates };
...
async function cancelarReceita(id) {
  ...
  const updates = { status: "CANCELADA", canceladaEm: nowIso(), updatedAt: nowIso() };
  await db.ref(`receitasPrivadas/${currentUser.uid}/${id}`).update(updates);
  const r = { ...(receitasCache[id] || {}), ...updates };
...
function renderReceitas() {
  ...
  body.innerHTML = "";
  list.forEach((r, idx) => {
    const statusClass = r.status === "CANCELADA" ? "red" : String(r.status).includes("ASSINADA") ? "green" : "amber";
    const inputId = `fileSigned_${idx}_${r.id.replace(/\W/g,"")}`;
    const tr = document.createElement("tr");
    tr.innerHTML = `
      <td><strong>${safeText(r.id)}</strong><br><span class="badge ${statusClass}">${safeText(r.status)}</span></td>
      <td>${safeText(r.paciente?.nome || "-")}<br><small>${safeText(r.tipo || "")}</small></td>
      <td>${fmtDate(r.dataEmissao)}</td>
      <td>${r.hashSha256 ? `<small>${safeText(r.hashSha256.slice(0,18))}...</small>` : `<span class="badge amber">sem hash ICP</span>`}</td>
      <td>
        <div class="actions">
...
function wireEvents() {
  ...
  $("#loginSenha").addEventListener("keydown", e => { if (e.key === "Enter") signIn(); });
}
